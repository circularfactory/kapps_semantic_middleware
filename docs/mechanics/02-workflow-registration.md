...
Every Workflow realizes exactly one Capability. An Operation addressed to that Capability resolves to this Workflow through the chain `Operation → implementsCapability → realizedByWorkflow`. See `05-operation-coordination.md` for how Operations are dispatched and executed.
...
- **`svc:address`** on the Service individual — the base URL of this middleware instance
