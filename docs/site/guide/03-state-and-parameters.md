...
`@mw.state` takes `capability_class` and `state_property_class` as **keyword-only** IRIs, both required, plus an optional `name`. Both classes must pre-exist in the ontology. See {doc}`02-workflow-registration` for the registration rules the decorator shares with `@mw.workflow`.
